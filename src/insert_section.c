...
static int32_t create_section(t_elf *elf, uint16_t last_section, uint16_t last_ptload) {
    ...
    memmove(new_shdrs + last_section + 2, new_shdrs + last_section + 1,
            sizeof(Elf64_Shdr) * (elf->elf_header->e_shnum - last_section - 2));
    memmove(new_sects + last_section + 2, new_sects + last_section + 1,
            sizeof(uint8_t *) * (elf->elf_header->e_shnum - last_section - 2));
